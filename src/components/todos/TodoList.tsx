  ...
  return (
    ...
    >
      <AccordionSummary
        expandIcon={
          !done ? (
            <ExpandMoreRoundedIcon sx={{ color: "text.disabled", fontSize: "1.1rem" }} />
          ) : undefined
        }
        sx={{
          minHeight: 48,
      ...
      >
        ...
        <Typography
          variant="caption"
          sx={{ color: "text.disabled", whiteSpace: "nowrap", fontSize: "0.7rem", mr: 0.5 }}
        >
          {dateStr}
        </Typography>
        <IconButton
  ...
  return (
    <Box sx={{ p: 4, maxWidth: 800, mx: "auto" }}>
      ...
      {!isLoading && pending.length > 0 && (
        <Box sx={{ display: "flex", flexDirection: "column", gap: 1, mb: done.length > 0 ? 3 : 0 }}>
          {pending.map((todo) => (
            <TodoAccordion
              key={todo.id}
              todo={todo}
