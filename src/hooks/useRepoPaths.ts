...
export function useRepoPaths() {
  ...
  const saveMutation = useMutation({
    mutationFn: async ({ repoFullName, localPath }: { repoFullName: string; localPath: string }) => {
    ...
    onMutate: async ({ repoFullName, localPath }) => {
      ...
      queryClient.setQueryData<RepoPathRow[]>(QUERY_KEY, (old = []) => {
        ...
        return [...old, { id: crypto.randomUUID(), repo_full_name: repoFullName, local_path: localPath }];
      });
