...
function updateDashboardIssues(old: DashboardData | undefined, params: UpdateStatusParams): DashboardData | undefined {
  if (!old) return old;
  return {
    ...old,
...
export function useUpdateIssueStatus() {
  ...
  return useMutation({
    ...
    onMutate: async (params) => {
      ...
      const queries = queryClient.getQueriesData<DashboardData>({ queryKey: DASHBOARD_KEY_PREFIX });
      ...
      queryClient.setQueriesData<DashboardData>(
        ...
        (old) => updateDashboardIssues(old, params)
      );
