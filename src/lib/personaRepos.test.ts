...
const FRONT = 'ludovic/kepler';
const INFRA = 'ludovic/infra';
...
describe('resolveActiveRepo', () => {
	...
	it('falls back to ALL_REPOS when the repo path is gone', () => {
		expect(resolveActiveRepo('ludovic/deleted', REPOS)).toBe(ALL_REPOS);
...
describe('reposOfPersona', () => {
	...
	it('ignores repos that are no longer configured', () => {
		expect(reposOfPersona(persona('p-ghost', ['ludovic/gone']), REPOS)).toEqual([]);
	});
});

describe('shortRepoName', () => {
	it('drops the owner', () => {
		expect(shortRepoName('ludovic/kepler')).toBe('kepler');
