...
describe('mergeConnectedBoards', () => {
	const mk = (node: string, col: string): GitHubIssue =>
		({
			...issue(node, 'open'),
			project_columns: [{ project: 'p', column: col }],
		}) as GitHubIssue;

	it('union des statusColumns dans l’ordre des configs, dédupliquée', () => {
		const { statusColumns } = mergeConnectedBoards([
			{
				config: { org: 'o', projectNumber: 1, statusColumns: ['Todo', 'Done'] },
				boardIssues: [],
			},
			{
				config: { org: 'o', projectNumber: 2, statusColumns: ['Todo', 'QA'] },
				boardIssues: [],
			},
		]);
	...
	it('tague chaque issue avec sa config et dédup cross-board par node_id', () => {
		const { issues } = mergeConnectedBoards([
			{
				config: { org: 'o', projectNumber: 1, statusColumns: [] },
				boardIssues: [mk('a', 'Todo')],
			},
			{
				config: { org: 'o', projectNumber: 2, statusColumns: [] },
				boardIssues: [mk('a', 'Todo'), mk('b', 'QA')],
			},
		]);
